...
def Macro2():
    ...
    p1.setElementType(regions=pickedRegions, elemTypes=(elemType1, elemType2,
        elemType3))
    p1.seedPart(size=4.0, deviationFactor=0.1, minSizeFactor=0.1)
    p1.generateMesh()
    ##齿轮2网格划分
    ...
    p2.setElementType(regions=pickedRegions, elemTypes=(elemType1, elemType2,
        elemType3))
    p2.seedPart(size=4.0, deviationFactor=0.1, minSizeFactor=0.1)
    p2.generateMesh()
    ...
    for face in faces:
        ...
        if abs(normal[2]) < 0.5 and abs(center[0]) > tol:
            gear_teeth_faces.append(face.index)

    # 构造 mask 字符串：将索引转换成 bitmask
    mask_int = 0
    for idx in gear_teeth_faces:
        mask_int |= (1 << idx)

    # 转成十六进制字符串表示（比如 [#1]、[#4004]）
    mask_str = f"[#{mask_int:X}]"

    gear_teeth_seq = faces.getSequenceFromMask(mask=(mask_str,))

    region1_2 = regionToolset.Region(side1Faces=gear_teeth_seq)
    ...
    region2_2 = regionToolset.Region(side1Faces=gear_teeth_seq)
    ...
    mdb.models['Model-1'].ImplicitDynamicsStep(name='Step-1', previous='Initial',
        timePeriod=0.01, maxNumInc=1000, initialInc=1e-04, minInc=1e-07,
        nlgeom=ON)
    ...
    try:
        ...
        pass
    mdb.Job(name='ZZ35', model='Model-1', description='', type=ANALYSIS,
    ...
    mdb.jobs['ZZ35'].submit(consistencyChecking=OFF)
    mdb.jobs['ZZ35'].waitForCompletion()
    print('Completed')
